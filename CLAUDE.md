...
- **Retention**: [`sql/retention_return_rate.sql`](sql/retention_return_rate.sql) over `hub_visits` ([`hub_visits_migration.sql`](hub_visits_migration.sql), anonymous per-browser UUID, insert-only from the browser, read only with the service role) reports the share of card-N visitors who came back for card N+1. Plausible cannot answer that.
