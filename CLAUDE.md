...
### Event Hub, fight pages, Market Board (September 2026 — the product is the next card)

The product is the NEXT UFC card. Order of importance: Event Hub > fight pages > Market Board > Fight Week Brief > Proof Center > Factor Lab > fighter pages.

- **Data layer** ([`fight_week_views.sql`](fight_week_views.sql)): `v_fight_locked_forecast` (the CFL number — from `pre_fight_snapshots`, or the insert-only `model_picks` live row until the snapshot exists; **never recomputed at render**), `v_fight_market_vigfree` (each sportsbook's pair de-vigged on its own, median across books, with `book_count` and `last_updated`), `v_fight_market_at_lock` (same, using only quotes captured at or before `locked_at`), `v_fight_odds_latest_by_book` (per-book latest quote, current card window). Sportsbooks only — the synthetic "CFL Consensus" row and prediction markets (Polymarket, Kalshi) are excluded; mirrored in [`books.js`](books.js). `v_fight_odds_consensus` is untouched.
- **Event Hub** = the `/e/<slug>-<id>.html` page of any card with a locked record, generated by [`build/hub-templates.js`](build/hub-templates.js) from [`build/prerender.js`](build/prerender.js) (cards without a record keep the redirect stub). After the card settles the same URL is regenerated as the permanent graded record — **never delete or redirect a hub URL**. **Fight pages** for those cards live at `/preview/<a>-vs-<b>-<id>.html` (same template file); legacy previews remain for cards with no locked forecast. `/card/` pages of hub events become redirects to the hub.
- **Market Board** ([`market-board.html`](market-board.html)) is client-rendered from the same views: largest disagreements, price watch (only Tennessee-licensed books in `books.js` get a link or the "best price" label; price ranking uses price only; no affiliate data anywhere), market movement since lock.
- **Homepage** hero = the next card with a locked forecast (fallback: latest graded card), top 3 disagreements, what changed today, the Card Lab, the record split three ways from `data/claims.json` (VERIFIED claims only), the Fight Week Brief block, about/methodology.
- **Shared code**: [`fight-week-core.js`](fight-week-core.js) (pure helpers, browser + Node — labels, the two standard lines, card assembly), [`fight-week.js`](fight-week.js) (client: hydration of hub/fight pages, the Brief signup block, Plausible events, the anonymous `hub_visits` ledger), [`build/fight-week-data.js`](build/fight-week-data.js) (Node reads). Bump `ASSET_V` in `hub-templates.js` and the `?v=fw…` on `market-board.html` / `index.html` when `fight-week.js` or `fight-week.css` change.
- **Wording (hard rule)**: the gap between CFL and the market is a *disagreement*. Never write "edge", "lock", "best bet", "value bet" or anything implying profit; every disagreement block carries "A disagreement is not a proven betting edge." and the standard line "CFL publishes model forecasts and market analysis, not handicapper picks." Display thresholds live in `fight-week-core.js` (`AGREE_POINTS` 5, `BIG_POINTS` 10, `HUGE_POINTS` 25) — they label a gap, they are not model thresholds.
- **Fight Week Brief**: one block (`.fw-brief`, rendered by `fight-week.js`) on the homepage, Event Hub and fight pages; saves `email_subscribers.source` as `<page_type>:<block_id>`. [`build/send-digest.js`](build/send-digest.js) sends the pre-card (Wednesday) and post-card (Monday; biggest miss at the same size as the biggest hit) emails; no sportsbook names or links in email — the sender refuses to send if one appears.
- **Retention**: [`sql/retention_return_rate.sql`](sql/retention_return_rate.sql) over `hub_visits` ([`hub_visits_migration.sql`](hub_visits_migration.sql), anonymous per-browser UUID rotated every 60 days, insert-only from the browser, read only with the service role, rows pruned after 120 days by `hub_visits_prune()` via `.github/workflows/hub-visits-prune.yml`) reports the share of card-N visitors who came back for card N+1. Plausible cannot answer that. The Tennessee-licensed book list in `books.js` carries its regulator source and check date (`TN_LIST_SOURCE` / `TN_LIST_CHECKED`) — re-check the page when a new book shows up in `odds_books`.
