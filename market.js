/* ==========================================================================
   ...
   page that formats its own market numbers can quietly drop one:
     ...
     2. `open_p_a` is OUR FIRST CAPTURE, not the opening line. It is always
        rendered with how many books were quoting at that moment, because on
        most cards the first capture is one offshore book and a reader who is
        not told that will read a book-mix change as a line move.
...
(function () {
  ...
  M.readSide = function (row, side, opts) {
    ...
    const open = num(isA ? row.open_p_a       : row.open_p_b);
    ...
    return {
      ...
      openProb: open,
      booksAtOpen: row.books_at_open == null ? null : +row.books_at_open,
      firstSeenAt: row.first_seen_at,
      moveSinceOpen: (open == null || now == null) ? null : (now - open) * 100,
  ...
  M.moveWords = function (pts, name) {
    if (pts == null) return { text: 'no earlier capture to compare', dir: 'none', notable: false };
  ...
  // The provenance sentence that must accompany any "since our first capture"
  // number. Kept here so no page can print the number without the caveat.
  M.openCaveat = function (sideRead) {
    if (!sideRead || sideRead.openProb == null) return '';
    const n = sideRead.booksAtOpen;
    const when = M.fmtStampUtc(sideRead.firstSeenAt);
    const books = n == null ? 'an unknown number of books'
                : n === 1   ? '1 book'
                            : n + ' books';
    return 'Our first capture of this fight: ' + books + ', ' + when +
           '. That is when CFL started watching, not when the market opened.';
  };
